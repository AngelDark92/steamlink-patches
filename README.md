...
Use Morphe Manager 1.22 or newer with compatibility checks enabled for build-specific filtering. Manager 1.7 cannot distinguish APKs that share versionName `2.0.22`, and Expert mode may intentionally show incompatible patches. Morphe's `default` flag is global, so 3 exact-build dependency bundles provide version-aware recommendations while every individual patch remains default-off. Build 5002322 receives the final 7-patch set including OLED `final-balanced`; build 5002318 receives its existing native-XR-safe set; verified builds through 5002244 receive only the legacy conversion foundation. Builds 5002296 and 5002313 retain expert-selectable patches but no automatic bundle. **Appear on top (legacy)** and **Change package name** are never recommended.
...
> **[v1.11.1-dev.4](https://github.com/AngelDark92/steamlink-patches/releases/tag/v1.11.1-dev.4)**&nbsp;&nbsp;•&nbsp;&nbsp;`dev`&nbsp;&nbsp;•&nbsp;&nbsp;23 patches total
<details open>
...
| [Galaxy XR legacy foundation (through 2.0.22/5002244)](#galaxy-xr-legacy-foundation-through-20225002244) | Recommended dependency bundle for the exact verified legacy builds through 5002244. | 5001712, 5001740, 5002172, 5002206, 5002244 |  |
| [Galaxy XR recommended set (2.0.22/5002318)](#galaxy-xr-recommended-set-20225002318) | Recommended native-XR-safe dependency bundle for exact build 5002318. | 5002318 |  |
| [Galaxy XR recommended set (2.0.22/5002322)](#galaxy-xr-recommended-set-20225002322) | Recommended final 7-patch dependency bundle for exact build 5002322, including OLED `final-balanced`. | 5002322 |  |
...
| [Visual Delay Fix](#visual-delay-fix) | Adds a configurable offset to the HMD OpenXR pose-query time and zeroes all six exported HMD velocity fields. Its trampoline uses a dedicated executable mapping over non-runtime ELF comment bytes and preserves live PLT entries. | 5001712, 5001740, 5002172, 5002206, 5002244, 5002313, 5002318, 5002322 | • Pose offset (ms) |
