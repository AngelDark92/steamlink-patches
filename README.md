...
4. Select it with Morphe. For builds 5002318/5002322, prefer normal compatibility-filtered mode; Expert mode can show incompatible legacy patches. Enable only the listed native-build patches (plus an optional projection experiment). Galaxy XR native telemetry is optional and requires an explicit PC LAN IPv4 plus a private pairing token matching the Windows host. Its precompiled bridge embeds that token in the patched APK, so keep the APK private and never distribute it.
