...
The [2.0.23/5002363 audit](diagnostics/steamlink-5002363/README.md) records the original APK, native addresses, patch scope, option checks and remaining headset validation. Its 7 individual adaptations remain available; decoder input buffering, UDP receive buffering and the FEC duplicate guard are additional standalone experiments with separate outcome records above. The 14 legacy patches remain excluded because this base already uses Valve's native Android XR paths.
