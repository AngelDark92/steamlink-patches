...
The first two live `three-projection-sampler-proxy-v1` captures are invalid as quality evidence. v1 disabled on a zero-layer transition. v1.1 primed all six proxies, but one-shot setup pushed Steam Link into its non-target spinner path and it disabled before any proxy transform. Both `LOW0` observations came after original-submission fallback. Build `three-projection-sampler-proxy-v1.2-20260829` now learns without allocating, stages one proxy per later target frame, preserves readiness across unrelated auxiliary layers, refreshes complete source updates that coincide with an auxiliary frame, and activates only after a successful rewritten `xrEndFrame`. Its collector proves activation before a device-clock-bounded visual interval and excludes teardown UI from quality provenance. It is statically validated but still needs a new headset run.
