...
The 2026-08-29 v1.2 rerun was stable for 5772 successful transformations and stayed `HIGH0 -> HIGH -> HIGH1`. On 2026-08-30, no remaining text or edge softness is visible, and reconstruction appears equal to the normal APK with the granted appear-on-top control when the validated `Highest` VRLink host profile is active. A later settings A/B proved that removing that profile lowered the effective VRLink render target from `3552x3840` to `2048x2048` and its transport target from fixed `1536` to automatic `1152`; the reconstruction patch cannot recover source pixels discarded on the PC/transport path. SteamVR still reported the same `3756x4064` current-application render target in both sessions, so the immediate observed downgrade occurred downstream in VRLink's render/encode path rather than in application supersampling. The 3745x4048 detail-preserving request was capped by Android XR at 3152x3682; this remains measured telemetry but is not evidence of a visible quality loss. The runtime did not advertise `XR_FB_composition_layer_settings`, so no legal compositor supersampling hint could be attached.
