...
object Sdr10ShaderAssembleAudit {
    ...
    private val bases = listOf(
        ...
        Base("2.0.20", "5001740", 2_220_528, "5fbb76c06c9fc0e3e5c5825752aa17e040462c8551b69d3492265f620244f443"),
        Base("2.0.22", "5002244", 2_251_920, "4b2fa5e1b5d9d5c938873f692b0e5e18159e1199dee1253dd6eccc8fa43dfa12"),
        Base("2.0.22", "5002313", 2_276_872, "e4d3575a130dc013e4c8fe4fb965217028229f89b13ba821c01b492e457398bb"),
        Base("2.0.22", "5002318", 2_277_488, "3c8d1ce13fd61edff5ce65efe6eedcc8565c89b66bab371550986a5c75407e56"),
        Base("2.0.22", "5002322", 2_283_400, "e61baf34dfc4749d92561bab5fee47891d271607a0ce44824ff61c3e6a450c3f"),
    ...
    fun main(args: Array<String>) {
        ...
        for (base in bases) {
            ...
            for (dither in VideoDitherMode.entries) {
                ...
                checkAssembled(opaque, stockInterface, "${base.code}/${dither}/opaque")
                checkAssembled(mask, stockInterface, "${base.code}/${dither}/masked")
                File(output, "${base.code}-neutral-${dither.optionValue}.opaque.glsl").writeText(opaque)
                File(output, "${base.code}-neutral-${dither.optionValue}.masked.glsl").writeText(mask)
                rows += "${base.code}/${dither.optionValue}: opaque=${opaque.length}B masked=${mask.length}B"
            }
            passed++
