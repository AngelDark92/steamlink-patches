...
object FecDuplicateReservationApkAudit {
    ...
    private val selections = setOf("baseline", "baseline-observe", "standalone", "bundle-first",
        "guard-first", "observe-guard-last", "guard-first-observe", "reapply")

    @JvmStatic
    fun main(args: Array<String>) = runBlocking {
        require(args.size in 6..7) {
            "Usage: FecDuplicateReservationApkAudit <archive.mpp> <input.apk> <version> <code> " +
        ...
        val observeCase = selection in setOf("baseline-observe", "observe-guard-last", "guard-first-observe")
        ...
        check(File(FecDuplicateReservationApkAudit::class.java.protectionDomain.codeSource.location.toURI())
            .canonicalFile == archive) { "Audit code is not loaded from the requested MPP" }
        val patchClass = Class.forName("app.template.patches.steamlink.binary.FecDuplicateReservationGuardPatchKt")
        check(File(patchClass.protectionDomain.codeSource.location.toURI())
            .canonicalFile == archive) { "FEC guard patch code is not loaded from the requested MPP" }
        ...
        check(!fecDuplicateReservationGuardPatch.default && fecDuplicateReservationGuardPatch.dependencies.isEmpty()) {
            "FEC guard patch must remain default-off with no dependencies"
        }
        ...
        val selected: Array<Patch<*>> = when (selection) {
            ...
            "standalone", "reapply" -> arrayOf(fecDuplicateReservationGuardPatch)
            "bundle-first" -> arrayOf(bundle, fecDuplicateReservationGuardPatch)
            "guard-first" -> arrayOf(fecDuplicateReservationGuardPatch, bundle)
            "observe-guard-last" -> arrayOf(bundle, decoderInputBufferingPatch, fecDuplicateReservationGuardPatch)
            else -> arrayOf(fecDuplicateReservationGuardPatch, bundle, decoderInputBufferingPatch)
        }
        val metadataGuard = rawResourcePatch(name = "FEC guard audit exact input guard", default = false) {
            execute {
        ...
        val stages = selected.map { selectedPatch ->
            rawResourcePatch(name = "FEC guard audit completed stage: ${selectedPatch.name}", default = false) {
                dependsOn(selectedPatch)
                execute { stageExecution += selectedPatch.name }
            }
        }
        val casePatch = rawResourcePatch(name = "FEC guard archive audit: $selection", default = false) {
        ...
        val beforeGuard = when (selection) {
        ...
        val expectedScene = if (baselineCase) beforeGuard else patchFecDuplicateReservationGuard(beforeGuard, version, code)
        val independentlyExpected = beforeGuard.copyOf().also {
            if (!baselineCase) guardedNop.copyInto(it, guardOffset(code))
        }
        check(expectedScene.contentEquals(independentlyExpected)) {
            "Production helper result differs from independent four-byte guard edit"
        }
        checkStockUdp(beforeGuard, code)
        val actualScene = ZipFile(output).use { zip ->
            zip.contentNames()
            if (baselineCase) {
                if (observeCase) {
                    val resource = decoderHelperResource(code, OBSERVE_MODE)
                    val helper = requireNotNull(FecDuplicateReservationApkAudit::class.java.getResourceAsStream(resource))
        ...
        checkStockUdp(actualScene, code)
        ...
        check(beforeGuard.size == actualScene.size) { "Scene size changed" }
        val changedOffsets = beforeGuard.indices.filter { beforeGuard[it] != actualScene[it] }
        if (!baselineCase) {
            val expectedOffsets = if (selection == "reapply") emptyList()
                else (guardOffset(code) until guardOffset(code) + 4).toList()
            check(changedOffsets == expectedOffsets) { "Unexpected FEC guard mutation offsets: $changedOffsets" }
        }
        ...
        val summary = linkedMapOf<String, Any?>(
            ...
            "guard_changed_offsets" to changedOffsets.map { "0x${it.toString(16)}" },
            ...
            "stock_udp_receive_buffer_preserved" to true,
            "independent_guard_expected_bytes" to "1f2003d5",
    ...
    private val guardedNop = byteArrayOf(0x1f, 0x20, 0x03, 0xd5.toByte())
    private fun guardOffset(code: String): Int = when (code) {
        "5002322" -> 0x167094
        "5002363" -> 0x167f60
        else -> error("Unsupported exact build code")
    }

    private fun checkStockUdp(bytes: ByteArray, code: String) {
        val offset = when (code) {
            "5002322" -> 0x1745a8
            "5002363" -> 0x1757a8
            else -> error("Unsupported exact build code")
        }
        check(bytes.copyOfRange(offset, offset + 4)
            .contentEquals(byteArrayOf(0x08, 0x02, 0xa0.toByte(), 0x52))) {
            "FEC guard audit requires stock 1 MiB UDP receive-buffer request"
        }
    }
    ...
    private fun compareOnlySceneChange(reference: File, output: File, version: String, code: String,
        ...
        ZipFile(reference).use { original -> ZipFile(output).use { patched ->
            ...
            check(patchedScene.contentEquals(patchFecDuplicateReservationGuard(originalScene, version, code))) {
                "Scene differs from FEC-guard-only change to reference"
            }
            val independent = originalScene.copyOf()
            guardedNop.copyInto(independent, guardOffset(code))
            check(patchedScene.contentEquals(independent)) { "Reference comparison exceeds the independent four-byte guard edit" }
            checkStockUdp(originalScene, code)
            checkStockUdp(patchedScene, code)
