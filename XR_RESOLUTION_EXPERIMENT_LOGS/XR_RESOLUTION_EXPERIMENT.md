...
- **Experimental Native Single-Projection Resolution + 10-bit Probe** uses the same CPU-optimized, dual-format native reconstruction and adds bounded resolution, allocation, attachment-precision, decoder-buffer, capability, topology, and `xrEndFrame` telemetry.

All remaining native candidates reconstruct 1 ordinary stereo projection with the optimized centered-sample draw. They change the interception/submission boundary; they do not make Valve's renderer directly generate a single projection. The dual-format variants do not force 10-bit: the existing Video output precision path must first make all 6 Valve source swapchains RGB10_A2. The unoptimized and all quad-view patches have been removed.
...
- installs exactly 1 of `single_projection_reconstruction_efficient_v1`, `single_projection_native_renderer_v1`, `single_projection_native_renderer_dual_v1`, or `single_projection_native_probe_v1`;
...
The dual-format variants accept only a uniform set of 6 sRGB8 or 6 RGB10_A2 source swapchains and create matching-format scratch and final swapchains after confirming runtime support. The diagnostic probe additionally records the decoder output description and AHardwareBuffer allocation contract, all 6 source swapchain formats, actual GLES attachment component sizes, the final output format and rectangles, and the submitted layer/view count. This can prove or contradict the app-side 10-bit chain. It cannot prove that the private Android XR compositor or panel retained 10-bit; that remains `INCOMPLETE` without external compositor/display evidence.

For resolution discovery, the probe records each eye's recommended and maximum dimensions and computes the density-preserving request. It then performs short-lived, allocation-only swapchain trials above the reported maximum and destroys them immediately. A successful allocation is evidence that the runtime accepts the object, not that it accepts or samples a larger submitted projection; the validator therefore keeps that result `INCOMPLETE`. Production submission stays clamped to reported OpenXR limits. `XR_FB_foveation` and vendor GL foveation support are logged but not enabled because foveation reduces shading work and does not increase the compositor's declared pixel capacity. A future larger-submission experiment is justified only if the live probe shows a reproducible accepted allocation and the ordinary clamped submission is otherwise clean.
...
cmake --build extensions\resolution-trace-layer\build-android-new --target gxr_single_projection_native_renderer_dual_v1
