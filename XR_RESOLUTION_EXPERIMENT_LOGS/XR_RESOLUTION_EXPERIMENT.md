...
Single-projection reconstruction v1.2 produced 5772 successful 3-to-1 transformations and stayed `HIGH0 -> HIGH -> HIGH1` without `SYSTEM_ALERT_WINDOW`. On 2026-08-30, after further direct comparison, the user reported that the original 3-layer stream with a visible granted overlay remains sharper than the reconstructed output even with the validated `Highest` VRLink host profile. The later exact-build native-hook A/B also reached the high path but retained the same slightly lower-resolution appearance and visible pixel-crawl/dither-like motion, ruling out implicit API-layer registration as the main source. The working host profile produced a `3552x3840` VRLink render target and fixed `1536` transport target, while removing it produced `2048x2048` and automatic `1152` respectively. SteamVR's current-application render target remained `3756x4064` in both observed sessions, isolating VRLink's downstream render/encode selection, especially the `1536` versus `1152` transport target, as one visual discriminator. The six 1536x1536 MSAA2 sources produced a calculated 3745x4048 detail-preserving request that the runtime capped at 3152x3682; the foveal inset therefore occupies about 1293x1397 pixels in the reconstructed output instead of 1536x1536. This measured precomposition loss is the strongest explanation for the remaining difference. Code audit also found that reconstruction previously left fixed-function `GL_DITHER` uncontrolled; because OpenGL ES enables it by default, the new draw could inherit dithering independently of Valve's shader option. Efficient v1.2 and native helper v1.1 now record the inherited state, disable it only during reconstruction, and restore it afterwards; this correction has not yet received a headset A/B. The captured runtime reported `qualityExtensionAdvertised=false`, so `XR_FB_composition_layer_settings` could not legally be attached in that run.
